                ...
                <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[1.1rem] bg-zinc-950">
                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain scroll-smooth" tabindex="-1">
                        ...
                        <template x-if="tab === 'map'">
                            <div
                                x-data="pdaWorldMap(@js($mapData), @js($currentCityId))"
                            ...
                            >
                                ...
                                <div
                                    ...
                                    class="pointer-events-none absolute z-10 rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs shadow-lg"
                                >
                                    <div class="font-semibold text-zinc-100" x-text="tooltip.label"></div>
                                    <div class="text-zinc-500" x-text="tooltip.country"></div>
                                    <div class="mt-0.5 italic text-zinc-500" x-text="tooltip.biome"></div>
                                </div>

                                {{-- Legend --}}
                                <div class="flex flex-wrap items-center gap-4 border-b border-zinc-800/60 px-4 py-2.5 text-xs">
                                    <div class="flex items-center gap-1.5">
                                        <div class="h-2.5 w-2.5 rounded-full bg-emerald-500 ring-1 ring-emerald-700"></div>
                                        <span class="text-zinc-500">Your location</span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                        <div class="h-2.5 w-2.5 rounded-full bg-zinc-600"></div>
                                        <span class="text-zinc-500">City</span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                        <svg width="22" height="10" aria-hidden="true">
                                            <line x1="2" y1="5" x2="16" y2="5" stroke="currentColor" stroke-width="1.5" class="text-zinc-600" />
                                            <polygon points="16,2.5 21,5 16,7.5" fill="currentColor" class="text-zinc-600" />
                                        </svg>
                                ...
                                <div class="relative w-full" x-ref="container">
                                    ...
                                    >
                                        <defs>
                                            <marker id="pda-arrow-default" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
                                                <path d="M0,0 L0,6 L8,3 z" class="fill-zinc-600" />
                                            </marker>
                                            <marker id="pda-arrow-current" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
...
<script>
function pdaWorldMap(mapData, currentCityId) {
    return {
        mapData,
        currentCityId,
        nodes: [],
        ...
        init() {
            this.width = this.$refs.container?.offsetWidth || 400;
            this.height = Math.max(300, Math.round(this.width * 0.65));
        ...
        layoutNodes(rawNodes, rawEdges) {
            ...
            rawNodes.forEach((n, i) => {
                ...
                pos[n.id] = {
                    x: this.width / 2 + (w / 2.5) * Math.cos(angle),
                    y: this.height / 2 + (h / 2.5) * Math.sin(angle),
                };
            });

            const edgeMap = {};
            rawEdges.forEach(e => { edgeMap[`${e.from}-${e.to}`] = true; });
            ...
            for (let iter = 0; iter < 200; iter++) {
                ...
                rawNodes.forEach(n => {
                    forces[n.id].x += (this.width / 2 - pos[n.id].x) * 0.005;
                    forces[n.id].y += (this.height / 2 - pos[n.id].y) * 0.005;
                });
            ...
            this.nodes = rawNodes.map(n => ({
                id: n.id, label: n.label, country: n.country, biome: n.biome,
                x: Math.round(pos[n.id].x), y: Math.round(pos[n.id].y),
            }));
        },
        ...
        computeEdges(rawEdges) {
            ...
            this.edges = rawEdges.map(e => {
                ...
                const offset = hasPair ? 4 : 0;
                const px = -dy / d * offset, py = dx / d * offset;
                return {
                    from: e.from, to: e.to,
                    x1: from.x + (dx / d) * r + px, y1: from.y + (dy / d) * r + py,
                    x2: to.x - (dx / d) * (r + 7) + px, y2: to.y - (dy / d) * (r + 7) + py,
                    isCurrent: e.from === this.currentCityId || e.to === this.currentCityId,
                };
            }).filter(Boolean);
        },
        ...
        renderSvg() {
            ...
            this.edges.forEach(edge => {
                edgesLayer.append(this.createSvgEl('line', {
                    x1: edge.x1, y1: edge.y1, x2: edge.x2, y2: edge.y2,
                    class: edge.isCurrent ? 'stroke-emerald-400' : 'stroke-zinc-600',
                    'stroke-width': 1.5,
                    'marker-end': edge.isCurrent ? 'url(#pda-arrow-current)' : 'url(#pda-arrow-default)',
                }));
            });

            this.nodes.forEach(node => {
                const isCurrent = node.id === this.currentCityId;
                const g = this.createSvgEl('g', { transform: `translate(${node.x},${node.y})`, class: 'cursor-pointer' });
                g.addEventListener('mouseenter', () => this.showTooltip(node));
                g.addEventListener('mouseleave', () => { this.tooltip.visible = false; });
                g.append(
                    this.createSvgEl('circle', {
                        r: isCurrent ? 12 : 8,
                        class: isCurrent ? 'fill-emerald-500 stroke-emerald-700' : 'fill-zinc-700 stroke-zinc-500',
                        'stroke-width': 2,
                    }),
                    Object.assign(this.createSvgEl('text', {
                        'text-anchor': 'middle', 'dominant-baseline': 'middle',
                        y: isCurrent ? 26 : 21, 'font-size': 9,
                        class: isCurrent ? 'fill-emerald-400' : 'fill-zinc-400',
        ...
        showTooltip(node) {
            ...
            this.tooltip = { visible: true, x: node.x * sx + 14, y: node.y * sy - 8, label: node.label, country: node.country, biome: node.biome };
        },
    };
}
</script>
